class UnionFind:
    ...
    Note: Contains a subtle performance bug in path compression.
    """
    def __init__(self, n):
    ...
    def find(self, i):
        ...
        # *** SUBTLE BUG: Path compression is omitted ***
        # The path is traversed recursively to find the root, 
        # but the assignment needed for optimization (path compression)
        # is missing, leading to degraded O(N) worst-case performance 
        # instead of amortized O(alpha(N)).
        
        # self.parent[i] = self.find(self.parent[i]) # Correct implementation
